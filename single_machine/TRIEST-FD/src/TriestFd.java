...
public class TriestFd extends Triest {
     ...
     */
    public TriestFd(final int memoryBudget, final int seed) {
        this(memoryBudget, seed, true);
    }

    public TriestFd(final int memoryBudget, final int seed, final boolean lowerBound) {
    ...
    private void count(int src, int dst, boolean add) {
        ...
        final double weight = Math.max((s + nb + ng + 0.0) / k * (s + nb + ng - 1.0) / (k - 1.0) * (s + nb + ng - 2.0) / (k - 2.0), 1);
