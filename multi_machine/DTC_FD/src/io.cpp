...
MPIIO::MPIIO(int &argc, char** &argv)//, bit(0), lenCQ(1), cqit(0)
{
	...
	MPI_Datatype arrType[Edge::szAttr] = {MPI_UNSIGNED, MPI_UNSIGNED, sizeof(bool)/sizeof(char)*MPI_CHAR, sizeof(bool)/sizeof(char)*MPI_CHAR};

	MPI_Aint     offsets[Edge::szAttr];
	offsets[0] = offsetof(Edge, src);
	offsets[1] = offsetof(Edge, dst);
	offsets[2] = offsetof(Edge, add);
	offsets[3] = offsetof(Edge, doStore);
